#!/usr/bin/env python3
"""
Ultra-simple visualization script for the Forge Stacker Insert environment.
This script loads only 1 environment with minimal settings for visualization.
"""
...
def visualize_simple():
    """Ultra-simple visualization of the stacker insert environment."""
    
    print("🎬 Loading Forge Stacker Insert Environment (Ultra-Simple Mode)...")
    ...
    try:
        ...
        # Reduce environment count to 1 and use CPU for everything
        cfg.num_envs = 1
        cfg.sim.physx.use_gpu = False  # Use CPU physics to avoid GPU memory issues
        cfg.sim.device = "cpu"  # Force CPU device
        print("✓ Using CPU physics and 1 environment only")
        
        # Create the environment with minimal settings - force CPU device
        ...
        step_count = 0
        max_steps = 200  # Run for 200 steps (about 2 seconds at 100Hz)
        
        while step_count < max_steps:
            # Take very small random actions
            actions = torch.randn((env.num_envs, env.action_space.shape[0]), device=env.device) * 0.01
